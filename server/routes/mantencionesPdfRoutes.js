...
router.get("/:id/pdf", auth, async (req, res) => {
  ...
  try {
    ...
    if (!rows.length) return res.status(404).send("Mantención no encontrada");
    const data = rows[0];

    // === CONSULTAR DETALLE DE TAREAS/REPUESTOS ===
    const [items] = await db.query(
      `
      SELECT item, tipo, cantidad, costo_unitario, (cantidad * costo_unitario) AS subtotal
    ...
    const drawHeader = (currentPage, totalPages) => {
      ...
      doc
        ...
        .fontSize(14)
        .fillColor("#003366")
        .text("INFORME DE MANTENCIÓN VEHICULAR", L, doc.y, {
          align: "left",
    ...
    if (items.length) {
      ...
      const headers = ["Ítem / Descripción", "Tipo", "Cantidad", "Costo Unitario", "Subtotal"];
      const rowHeight = 18;
      ...
      for (const [index, it] of items.entries()) {
        ...
        x = startX;
        doc.fillColor("#000")
          .text(it.item, x + 5, y + 4, { width: colWidths[0] - 10 });
        x += colWidths[0];
        doc.text(it.tipo || "-", x + 5, y + 4, { width: colWidths[1] - 10 });
        x += colWidths[1];
        doc.text(it.cantidad || "-", x + 5, y + 4, { width: colWidths[2] - 10 });
        x += colWidths[2];
        doc.text(`$${Number(it.costo_unitario).toLocaleString("es-CL")}`, x + 5, y + 4, { width: colWidths[3] - 10 });
        x += colWidths[3];
        doc.text(`$${Number(it.subtotal).toLocaleString("es-CL")}`, x + 5, y + 4, { width: colWidths[4] - 10 });
        y += rowHeight;
      ...
      doc.text(`TOTAL: $${total.toLocaleString("es-CL")}`, startX + 290, y + 10);
      doc.moveDown(3);
    } else {
    ...
    if (firmaY > doc.page.height - 120) doc.addPage();

    doc.text("_____________________________", L, doc.y + 50);
    doc.text("Firma Responsable", L + 15, doc.y + 62);

    doc.text("_____________________________", L + 250, doc.y + 50);
    doc.text("Supervisor", L + 270, doc.y + 62);
